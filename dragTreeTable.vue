<template>
  <div class="drag-tree-table" ref="table" v-bind:class="{border:border !== undefined}">
    <div class="drag-tree-rable-scroll" style="width: 100%;overflow: scroll;">
      ...
      <div class="drag-tree-table-body"
        ...
        <row depth="0" :columns="data.columns" :isdraggable="isdraggable"
          ...
          :key="index"
          @on-current-change="e => $emit('on-current-change', e)"
        >
          <slot v-for="(c,ci) in data.columns" :name="c.slot" />
        </row>
      </div>
      <div class="drag-line">
      </div>
    </div>
    <div v-if="leftFixedColumns.length" class="left-fixed-container">
      <div class="drag-tree-table-header">
        <column
          v-for="(item, index) in leftFixedColumns"
          :width="item.width"
          :flex="item.flex"
          :border="border === undefined ? resize : border"
          v-bind:class="['align-' + item.titleAlign, 'colIndex' + index]"
      ...
      <div class="drag-tree-table-body"
        ...
        @dragend="drop"
        :class="isDraing ? 'is-draging' : '' ">
        <row depth="0" :columns="leftFixedColumns" :isdraggable="isdraggable"
          :model="item"
          :index="index"
          v-for="(item, index) in data.lists"
          :custom_field="custom_field"
          ...
          @on-current-change="e => $emit('on-current-change', e)"
        >
          <slot v-for="(c,ci) in leftFixedColumns" :name="c.slot" />
        </row>
      </div>
      <div class="drag-line">
      </div>
    </div>
    <div v-if="rightFixedColumns.length" class="right-fixed-container">
      <div class="drag-tree-table-header">
        <column
          v-for="(item, index) in rightFixedColumns"
          :width="item.width"
          :flex="item.flex"
          :border="border === undefined ? resize : border"
          v-bind:class="['align-' + item.titleAlign, 'colIndex' + index]"
      ...
      <div class="drag-tree-table-body"
        ...
        @dragend="drop"
        :class="isDraing ? 'is-draging' : '' ">
        <row depth="0" :columns="rightFixedColumns" :isdraggable="isdraggable"
          :model="item"
          :index="index"
          v-for="(item, index) in data.lists"
          :custom_field="custom_field"
          ...
          @on-current-change="e => $emit('on-current-change', e)"
        >
          <slot v-for="(c,ci) in rightFixedColumns" :name="c.slot" />
        </row>
      </div>
      <div class="drag-line">
      </div>
...
export default {
  ...
  computed: {
    ...
    leftFixedColumns() {
      return this.data.columns.filter(item => item.fixed === 'left')
    },
    rightFixedColumns() {
      return this.data.columns.filter(item => item.fixed === 'right')
    },
  },
  props: {
    isdraggable: {
  ...
  methods: {
    ...
    resetOrder(list) {
      ...
      for (var i = 0; i< list.length; i++) {
          list[i][this.custom_field._order] = i;
    ...
    DelById(id) {
      const listKey = this.custom_field.lists
      const orderKey = this.custom_field._order
      const idKey = this.custom_field.id
      const newList = [];
      const curList = this.data.lists;       
      function pushData(curList, needPushList) {
      ...
      function deep(list) {
        ...
        for (var i = 0; i< list.length; i++) {
            if (list[i][_this.custom_field['id']] == pId) {
              ...
              if (list[i][listKey]) {
                newRow[_this.custom_field['_order']] = list[i][listKey].length;
                list[i][listKey].push(newRow)
              } else {
                list[i][listKey] = [];
                newRow[_this.custom_field['_order']] = 0;
                list[i][listKey].push(newRow)
              }
            }
            if (list[i][listKey] && list[i][listKey].length) {
...
.drag-tree-rable-scroll::-webkit-scrollbar {
  // display: none;
  z-index: 1;
  height: 8px;
}
.drag-tree-rable-scroll::-webkit-scrollbar-thumb {
  // display: none;
  z-index: 2;
  border-radius: 10px;
