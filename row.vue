<template>
  <div class="tree-block" :draggable="!!isdraggable" @dragstart="dragstart($event)"
    ...
    <div class="tree-row"
      ...
      @mouseover="$emit('on-current-change', model)"
      @mouseout="$emit('on-current-change', {})"
    > 
      <column
        ...
        <span v-if="subItem.type === 'selection'">
          <space :depth="depth"/>
          <span v-if = "model[custom_field.lists] && model[custom_field.lists].length" class="zip-icon" v-bind:class="[model[custom_field.open] ? 'arrow-bottom' : 'arrow-right']" @click="toggle" >
          </span>
          <span v-else class="zip-icon arrow-transparent">
          </span>
          <!-- <span v-if="subItem.formatter" v-html="subItem.formatter(model,index)"></span> -->
          <table-slot v-if="subItem.slot" :row="model" :index="index" :column="subItem" />
        ...
        <span v-else-if="subItem.type === 'action'">
          <a class="action-item"
            ...
            <i :class="acItem.icon" v-html="acItem.formatter(model,index)"></i>
          </a>
        </span>
...
export default {
  name: 'row',
    inject: ['tableRoot'],
    props: ['model','depth','columns','isdraggable','border', 'custom_field','onCheck','isContainChildren','index'],
    data() {
      return {
        open: false,
        visibility: 'visible'
